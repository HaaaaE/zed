...
- [partial 2026-05-30] `FencedCodeBlock`/`IndentedCodeBlock`：inactive 合成 code block item，隐藏 fence/info marker，保留 raw code 内容与换行；cursor 在 fence/info marker 时 reveal 该 source row。后续仍需接入 flow layout/visual row helper 来完整支持多视觉行命中与 selection bounds。
