...
- 2026-06-01:
  ...
  - `rendered_enter_delete_apply` perf 继续拆成 `rendered_enter_apply` 和 `rendered_delete_apply`，确认大文档 enter/delete 各自触发一次 syntax refresh：拆分前 enter parse 约 1.61s、delete parse 约 1.54s。
  - `markdown_wysiwyg` incremental parse 现在按旧 inline parent id 复用未触碰的 inline tree；`MarkdownParseTree::edit` 同步平移 inline parent range，使长度变化后编辑点之后的未触碰 inline tree 也能复用。
  - 新 perf 显示大文档 session 从约 32.45s 降到约 27.20s；`rendered_edit_length_change_draw_syntax_parse` 从约 1.57s 降到约 0.85s，`rendered_enter_apply_syntax_parse` 从约 1.61s 降到约 0.86s，`rendered_delete_apply_syntax_parse` 从约 1.54s 降到约 0.53s。
  - 剩余主要成本转到全量 block/inline/projection 收集：大文档普通 draw/enter/delete 的 projection 收集仍约 0.26-0.29s，blocks/inlines 各约 0.09-0.11s；下一步需要让 derived syntax collections 也按 dirty window 复用。
  - 已验证：`cargo fmt --check`、`cargo test -p markdown_wysiwyg`、`cargo test -p md_projection`、`cargo test -p md_editor rendered_plain_text_edits_in_common_rows_skip_syntax_and_full_index_build`、`cargo check -p updraft_editor`、`cargo test -p md_editor --profile release-fast --lib --no-run --config 'target."cfg(true)".rustflags=["--cfg","perf_enabled"]'`、`cargo perf-test -p md_editor -- --important`。
  - inline tree 复用增加 parent byte range fallback，不再只依赖 tree-sitter node id；配合已平移的 parent range，等长和长度变化编辑都能复用更多未触碰 inline tree。
