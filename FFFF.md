...
- 2026-06-02：pulldown/tree-sitter 等价覆盖已从底层语义数组扩展到查询 API 层，新增测试直接比较 `blocks_in_source_range()`、`inline_spans_in_source_range()`、`tables()`、`table_for_source_row()`、`table_for_source_range()`、`table_row_for_source_row()`、`projection_for_source_range()`、`active_projection_source_ranges_for_source_range()` 与 `range_semantics_for_source_range()` 的查询输出；第三十九轮验证 `cargo check -p markdown_wysiwyg --locked`、`cargo test -p markdown_wysiwyg --locked`、`cargo check -p updraft_editor --locked`、`cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml`、`RUSTFLAGS='--cfg perf_enabled' cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml` 全部通过，`git diff --check` 仅有 CRLF 提示。
- 2026-06-02：pulldown 增量路径的覆盖已从底层语义数组扩展到查询 API 层，新增测试在 heading、inline strong、image URL、blockquote text 与 table-cell inline code 的本地编辑后，直接比较 pulldown incremental 输出和 tree-sitter baseline 的 `blocks_in_source_range()`、`inline_spans_in_source_range()`、table 查询、projection 查询、active projection ranges 与 `range_semantics_for_source_range()`；第四十轮验证 `cargo check -p markdown_wysiwyg --locked`、`cargo test -p markdown_wysiwyg --locked`、`cargo check -p updraft_editor --locked`、`cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml`、`RUSTFLAGS='--cfg perf_enabled' cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml` 全部通过，`git diff --check` 仅有 CRLF 提示。
- 2026-06-02：pulldown/tree-sitter 查询 API 等价 helper 已继续覆盖 row-based 门面，full 与 incremental 查询测试现在同时比较 `source_range_for_rows()`、`projection_for_visible_rows()` 与 `range_semantics_for_visible_rows()`，避免只用 byte range 查询证明可见行路径；第四十一轮验证 `cargo check -p markdown_wysiwyg --locked`、`cargo test -p markdown_wysiwyg --locked`、`cargo check -p updraft_editor --locked`、`cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml`、`RUSTFLAGS='--cfg perf_enabled' cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml` 全部通过，`git diff --check` 仅有 CRLF 提示。
- 2026-06-02：pulldown/tree-sitter 查询 API 等价 helper 已补齐 inactive-ranges 投影入口，full 与 incremental 查询测试现在直接比较 `projection_for_source_range_with_inactive_ranges()`，覆盖 active source 与 inactive source ranges 同时参与投影过滤的路径；第四十二轮验证 `cargo check -p markdown_wysiwyg --locked`、`cargo test -p markdown_wysiwyg --locked`、`cargo check -p updraft_editor --locked`、`cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml`、`RUSTFLAGS='--cfg perf_enabled' cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml` 全部通过，`git diff --check` 仅有 CRLF 提示。
- 2026-06-02：pulldown 增量 full/incremental 一致性测试已加入结构性 marker 编辑场景，覆盖 ATX heading level、blockquote 内 heading level、table delimiter alignment marker 与 task checked marker 的本地编辑，确认结构语义变化后 incremental 输出仍和 pulldown full parse 一致；第四十三轮验证 `cargo check -p markdown_wysiwyg --locked`、`cargo test -p markdown_wysiwyg --locked`、`cargo check -p updraft_editor --locked`、`cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml`、`RUSTFLAGS='--cfg perf_enabled' cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml` 全部通过，`git diff --check` 仅有 CRLF 提示。
- 2026-06-02：pulldown 增量 full/incremental 一致性测试已扩展到 block kind 转换场景，覆盖 paragraph -> heading、pipe table delimiter 失效、HTML block -> paragraph 与 task list marker -> ordered list marker 的本地编辑；该覆盖暴露并修正了增量 `projection_marker_dependencies` 复用旧依赖的问题，现在 incremental 路径改为从当前 blocks / inline spans / projection replacements 重新派生 marker dependencies；第四十四轮验证 `cargo check -p markdown_wysiwyg --locked`、`cargo test -p markdown_wysiwyg --locked`、`cargo check -p updraft_editor --locked`、`cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml`、`RUSTFLAGS='--cfg perf_enabled' cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml` 全部通过，`git diff --check` 仅有 CRLF 提示。
- 2026-06-02：pulldown 增量后查询 API 等价测试已扩展到结构转换编辑，覆盖 paragraph -> heading、blockquote 内 list marker 从 unordered 改 ordered、pipe table delimiter 失效后，`blocks_in_source_range()`、table 查询、projection 查询、row-based 查询与 range semantics 仍和 tree-sitter baseline 对齐；第四十五轮验证 `cargo check -p markdown_wysiwyg --locked`、`cargo test -p markdown_wysiwyg --locked`、`cargo check -p updraft_editor --locked`、`cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml`、`RUSTFLAGS='--cfg perf_enabled' cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml` 全部通过，`git diff --check` 仅有 CRLF 提示。
- 2026-06-02：已跑更大规模真实 benchmark smoke：`MARKDOWN_SYNTAX_BENCH_BYTES=131072`、`MARKDOWN_SYNTAX_BENCH_ITERATIONS=1`、`RUSTFLAGS='--cfg perf_enabled' cargo run --manifest-path tooling/markdown_syntax_bench/Cargo.toml`；实际 fixture 为 131694 bytes，真实 pulldown semantics diff 仍为 `mismatch_fields=0`，其中 `projection_marker_dependencies` 为 11985 / 11985 全量匹配；第四十六轮仅记录验证证据。
- 2026-06-02：pulldown 增量 full/incremental 一致性测试已扩展到多行编辑场景，覆盖插入多行 paragraph/blockquote、删除 hard-break 段落片段、把 quoted fenced code 替换为多行 quoted paragraph、向 pipe table 追加一行等本地编辑，继续确认 line starts、row ranges、block/table/inline/projection 语义在 incremental 与 full parse 间一致；第四十七轮验证 `cargo check -p markdown_wysiwyg --locked`、`cargo test -p markdown_wysiwyg --locked`、`cargo check -p updraft_editor --locked`、`cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml`、`RUSTFLAGS='--cfg perf_enabled' cargo check --manifest-path tooling/markdown_syntax_bench/Cargo.toml` 全部通过，`git diff --check` 仅有 CRLF 提示。
