...
当前仓库原本是 Zed 的 fork。我们的目标不是长期维护一个完整的 Zed 分支，而是充分利用 Zed 已有的编辑器能力、UI 架构和实现经验，做出一个专注 Markdown 的文档编辑器。

当前阶段先做一个新的 Markdown-only 入口，把产品形态独立出来。等新入口和核心体验跑通后，再逐步删掉 Zed 中对这个 Markdown 编辑器不必要的功能、入口和依赖。
...
最终效果应该是一个轻量的 Markdown 文档编辑器，具备 Zed 质量的编辑体验和 Markdown 预览，并且逐步删除对这个目标不必要的 Zed 功能、入口和依赖。
