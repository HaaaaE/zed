...
impl MarkdownStructure {
    ...
    fn collect_blocks_from_parse_tree(&mut self, source: &str) {
        collect_structure_block_nodes(
            source,
            self.parser_state.block_tree().root_node(),
            &mut self.blocks,
        );
        self.blocks
            .sort_by_key(|block| (block.source_range.start, block.source_range.end));
    }
}
...
fn structure_block_from_node(
    ...
    match node.kind() {
        ...
        "html_block" => Some(MarkdownStructureBlock {
            ...
            marker_ranges: Vec::new(),
            row_range: row_range_for_structure_node(node),
            tagfilter_disallowed: inline::raw_html_tagfilter_disallowed(source, node.byte_range()),
        }),
...
fn structure_block_quote_block(
    source: &str,
    node: tree_sitter::Node<'_>,
...
fn structure_list_block(source: &str, node: tree_sitter::Node<'_>) -> MarkdownStructureBlock {
    let source_range = node.byte_range();
...
fn structure_list_item_block(source: &str, node: tree_sitter::Node<'_>) -> MarkdownStructureBlock {
...
fn structure_setext_heading_block(
    source: &str,
    node: tree_sitter::Node<'_>,
) -> Option<MarkdownStructureBlock> {
    let source_range = node.byte_range();
...
fn structure_atx_heading_block(
    source: &str,
    node: tree_sitter::Node<'_>,
) -> Option<MarkdownStructureBlock> {
    let source_range = node.byte_range();
...
fn structure_fenced_code_marker_ranges(node: tree_sitter::Node<'_>) -> Vec<Range<usize>> {
...
fn structure_pipe_table_marker_ranges(node: tree_sitter::Node<'_>) -> Vec<Range<usize>> {
    let mut marker_ranges = Vec::new();
    let mut cursor = node.walk();
    for child in node.children(&mut cursor) {
        match child.kind() {
...
fn structure_fenced_code_content_range(source: &str, node: tree_sitter::Node<'_>) -> Range<usize> {
