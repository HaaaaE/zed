...
- Inactive fenced code content rows now also use the block layout path through `DisplayBlockKind::FencedCode` and `DisplayBlockLayout::FencedCode`, with measured block height, block-row mouse targets, boundary carets, and cacheable row layouts.
...
- The shared block module now covers the first non-image block kind (`FencedCode`), including kind detection, block layout construction, rendering, and row-level boundary geometry reuse.
...
- Implement broader general block-level GPUI elements as measured list items or subitems. Remote image blocks and an initial fenced-code block path now exist, but arbitrary GPUI block measurement and richer block-kind coverage are not solved.
